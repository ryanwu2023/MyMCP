...
async def test_service_normalizes_inputs_and_maps_proposal_results() -> None:
    ...
    service = ShareholderMeetingService(repository)
...
@pytest.mark.asyncio
@pytest.mark.parametrize("wind_code", ["", "000001", "ABC.SZ", "000001.HK"])
async def test_service_rejects_invalid_wind_codes(wind_code: str) -> None:
    with pytest.raises(ValueError, match="Wind 股票代码"):
        await ShareholderMeetingService(FakeRepository()).get_shareholder_meetings(
            wind_code
        )
...
async def test_service_rejects_invalid_meeting_dates(meeting_date: str) -> None:
    with pytest.raises(ValueError, match="YYYYMMDD"):
        await ShareholderMeetingService(FakeRepository()).get_shareholder_meetings(
            "000001.SZ", meeting_date=meeting_date
        )
